...
NewProjectD::NewProjectD(){
    ...
    label->set_text("Enter Project Name");
    label->set_halign(Gtk::Align::START);
    label->set_css_classes({"dialog-label"});

    error = Gtk::manage(new Gtk::Label());
    error->set_text("Project Name already exists");
    ...
    cancelBtn->signal_clicked().connect(sigc::mem_fun(*this, &NewProjectD::hideUI));
...
void NewProjectD::showError(){
    error->set_visible(true);
}

void NewProjectD::showError(std::string message){
    error->set_text(message);
    error->set_visible(true);
}

void NewProjectD::hideError(){
    error->set_visible(false);
}

Glib::ustring NewProjectD::getEntryText(){
    return entry->get_text();
}


void NewProjectD::hideUI(){
    is_visible = false;
    set_visible(false);
}